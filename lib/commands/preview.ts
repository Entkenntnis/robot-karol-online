...
export function execPreviewForAll(core: Core) {
  for (let i = 0; i < core.ws.quest.tasks.length; i++) {
    execPreviewForTask(core, i)
  }
}

export function execPreviewForTask(core: Core, index: number) {
  const bytecode = core.ws.vm.bytecode
  if (!bytecode) return

  const world = JSON.parse(
    JSON.stringify(core.ws.quest.tasks[index].start)
  ) as World
  ...
  core.mutateWs(({ quest }) => {
    quest.tasks[index].target = world
  })
}
